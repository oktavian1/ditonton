...
void main() {
  ...
  late TvOnAirNotifier notifier;
  late int listenerCallCount;

  setUp(() {
    listenerCallCount = 0;
    mockGetOnAirTv = MockGetOnAirTv();
    notifier = TvOnAirNotifier(getOnAirTv: mockGetOnAirTv)
      ..addListener(() {
        listenerCallCount++;
      });
  });
  ...
  test('should change state to loading when usecase is called', () async {
    ...
    // assert
    expect(notifier.onAirTvState, RequestState.Loading);
  ...
  test('should change movies data when data is gotten successfully', () async {
    ...
    // assert
    expect(notifier.onAirTvState, RequestState.Loaded);
    expect(notifier.onAirTv, testTvList);
  ...
  test('should return error when data is unsuccessful', () async {
    ...
    await notifier.fetchOnAirTv();
    // assert
    expect(notifier.onAirTvState, RequestState.Error);
